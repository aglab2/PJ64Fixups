...
{
	static auto Name = stdstr_f("RSP %s", VER_FILE_VERSION_STR);
	return Name.c_str();
}
const char * AboutMsg ( void ) 
{
	static auto Msg = stdstr_f("RSP emulation plugin\nMade for Project64 (c)\nVersion %s\n\nby Jabo and Zilmar", VER_FILE_VERSION_STR);
	return Msg.c_str();
}
...
extern EXPORT void PluginLoaded(void);
...
EXPORT void GetDllInfo(PLUGIN_INFO * PluginInfo)
{
	...
	InitOnceExecuteOnce(&InitOnce, InitHandleFunction, NULL, NULL);
...
extern EXPORT void EnableDebugging(Boolean Enabled);
